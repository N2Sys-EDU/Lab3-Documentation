...
**本节内容是对Lab3习题课的总结和补充说明，请错过习题课的同学通过在线学习的方式先了解习题课的内容。**

### 0.1 选择eBPF的理由

传统的NAT和代理实现方式通常是通过用户态程序实现的。然而，用户态程序在处理网络报文时存在一定的性能瓶颈，主要体现在以下几个方面：
1. **上下文切换开销**：用户态程序需要频繁地在用户态和内核态之间切换，这会带来显著的性能开销，尤其是在高频率的网络报文处理场景下。
2. **内存拷贝开销**：用户态程序通常需要将报文从内核空间拷贝到用户空间进行处理，处理完成后再将报文拷贝回内核空间，这种频繁的内存拷贝操作会导致较高的延迟和资源消耗。

为了避免上述问题，研究人员逐步发展出了在内核态处理网络报文的技术eBPF（extended Berkeley Packet Filter）。这一技术能够让开发者编写的程序直接在内核态运行，从而大幅减少上述开销，提高网络报文处理的效率。

### 0.2 eBPF的限制

那么，性能提高伴随的代价是什么呢？
eBPF程序运行在**内核态**。我们在ICS和操作系统课程中都了解到，**内核态**的代码执行不当可能会导致系统崩溃。因此，eBPF引入了一系列限制来确保内核的安全性和稳定性：
1. **受限的指令集**：eBPF程序只能使用一组受限的指令集，这些指令经过精心设计，以防止程序执行可能危害内核稳定性的操作。
2. **严格的验证机制**：在eBPF程序加载到内核之前，内核会对程序进行严格的验证，确保程序不会执行无限循环或访问非法内存等操作。
3. **有限的资源访问**：eBPF程序只能访问有限的内核资源，不能直接操作内核数据结构。

这些限制注定了eBPF程序在功能上无法与用户态程序相提并论，能做到的事情也相对有限。不过，eBPF程序在网络报文处理方面已经足够强大，能够实现NAT和代理等功能。

回到此次Lab，这些限制意味着我们在实现NAT和代理功能时，必须使用**受限的指令集**并经过**验证器**的检验。尽管我们依然用C语言编写程序，但是我们需要遵守eBPF的编程规范，并确保程序能够通过验证器的检查。**值得注意的是，验证器并不足够强大，有时需要我们用更为保守的编程方式来规避验证器的误判。**
...
在本实验中，你可以默认测试中使用的必要的地址信息均为上述样例中使用的地址信息。即：
1. router在广域网的IP地址为10.0.0.2，代理服务端的IP地址为10.0.0.4；若使用代理服务，则服务器的IP地址一定是10.0.0.3
2. 代理服务器使用的端口为12345
3. 局域网网段为192.168.1.0/24
...
#define DST_IP 0x0300000a                               // 目标IP地址一定为 10.0.0.3（在不涉及到代理的测试点，也就是仅对NAT的测试中，可能目标IP地址不是10.0.0.3，这是为了防止同学们在NAT中把目标地址写死；但是对于代理，可以默认目标IP地址就是10.0.0.3）
...
由于`bpf_csum_diff`只支持__u32类型的输入，在修改的内容长度不足时，我们只需要填充我们的修改到4byte即可。以下是一个针对TCP校验和修改的例子：
