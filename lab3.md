...
在 Lab 3 中，你需要实现一个简单的运行 DV 算法并支持 NAT 和防火墙功能的路由器。
...
注意上面的`<type>`按顺序排列缺少编号4，原因是4预留作controller内部指令(参见5.4节debug模式的del_host)。
...
为了避免你刚上手时迷失在前文各种技术细节中，此处总结了router在各种情况下的转发行为以及相应的参考章节：
...
7. `blockaddr <router_id> <addr>` 要求对应的路由器阻止来自`<addr>`的报文
8. `unblockaddr <router_id> <addr>` 要求对应的路由器取消阻止来自`<addr>`的报文
...
>
> 提示：General类别的test网络拓扑规模较大，请注意实现方式以免在这些测试中超时。
